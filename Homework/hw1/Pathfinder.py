...
def solve(problem, initial, goals):
    ...
    frontier.put_nowait(
        (0, SearchTreeNode(initial, None, None, 0, h(initial, goals[1]))))

    while not frontier.empty():
        current = frontier.get_nowait()[1]
        for neighbor in MazeProblem.transitions(current.state):
            new_node = SearchTreeNode(neighbor[2], neighbor[0], current, current.totalCost + neighbor[1], h(neighbor[2])

            if neighbor[2] in graveyard:
                frontier.put_nowait(
